...
function test_input($data)
  ...
  $sql_search="SELECT * FROM tbl_item WHERE LOWER(Item_Name) LIKE LOWER('%$search%') OR LOWER(Description) LIKE LOWER('%$search%') OR SubCat_id IN (SELECT SubCat_id FROM tbl_subcat WHERE LOWER(SubCat_Name) LIKE LOWER('%$search%') OR Cat_id IN (SELECT Cat_id FROM tbl_category WHERE LOWER(Cat_Name) LIKE LOWER('%$search%')))";
  $sql_search_details=mysqli_query($conn,$sql_search);
...
<body>
  ...
  <!-- Item Cards -->
  <h4 style="text-align: center; margin-bottom: 50px; margin-top: 50px;">Search Result</h4>
  <div class="row row-cols-1 row-cols-md-4 g-0 " id="card1">

    <?php
                while($item=mysqli_fetch_assoc($sql_search_details))
                {
                  $c++;
        ?>
    ...
    <?php                  
        ...
        if($c==0)
                ...
                <div class="container" style="min-height: calc(100vh - 372px);">
                  <p class="text-muted" style="margin-left: 90px;">No Matching Results!</p>
                </div>
                  <?php
                  }
        ?>
