...
session_start();
$c=1;
$sql_fetch_vendor="SELECT * FROM tbl_vendor";
$sql_exe_vendor=mysqli_query($conn,$sql_fetch_vendor);
...
<html lang="en">
    ...
    <head>
        ...
        <title>GLOWUP| Vendor Details</title>
    ...
    ?>
        ...
        <div class="row d-flex flex-column flex-shrink-0 p-3 text-white container align-items-center " style="width: 250px; background-color:#97c1fc;">
            ...
            <ul class="nav nav-pills flex-column mb-auto">
              ...
              <li>
                <a href="Vendor.php" class="nav-link  active" style="background-color: rgb(50, 50, 50);" aria-current="page">
            ...
            <div class="row">
                <div class="col-md-10">
                      ...
                      <div class="panel-heading">
                        <div class="row">
                          ...
                          <div class="col col-xs-6" style="text-align: right;">
                            <a href="AddVendor.php" role="button" style="font-size: small;" class="btn btn-outline-dark">Add Vendor</a>
                          </div>
                        </div>
                      </div>
                    </div>
                    <a href="vendordownload.php" role="button"  class="btn btn-outline-dark mt-4 mb-3"><i class="bi bi-download"></i></a>
                    <table class="styled-table">
                      ...
                      <tbody>
                        <?php
                          while($row=mysqli_fetch_array($sql_exe_vendor))
                          {
                            $sid=$row['Staff_id'];

                            $sql_fetch_staff="SELECT * FROM tbl_staff WHERE Staff_id='$sid'";
                            $sql_exe_staff=mysqli_query($conn,$sql_fetch_staff);
                            $staff=mysqli_fetch_array($sql_exe_staff);
                        ?>

                        <tr> <th scope="row"><?php print_r($c++);?></th>
                          <th scope="row"><?php print_r($row['Vendor_id']);?></th>
                          <?php 
                            if(empty($staff))
                            {
                          ?>
                          ...
                          <?php
                            ...
                            {
                          ?>
                          <td><?php print_r($staff['S_FName']);?> <?php print_r($staff['S_LName']);?></td>
                          <?php
                            }
                          ?>
                          <td><?php print_r($row['V_Name']);?></td>
                          <td><?php print_r($row['V_Email']);?></td>
                          <td><?php print_r($row['V_PhNo']);?></td>
                          <td><?php print_r($row['V_BNo']);?></td>
                          <td><?php print_r($row['V_Street']);?></td>
                          <td><?php print_r($row['V_Dist']);?></td>
                          <td><?php print_r($row['V_pin']);?></td>
                          <td>
                            <a href="EditVendor.php?user_id=<?php echo $row['Vendor_id'];?>" role="button" style="font-size: small;" class="btn btn-outline-dark">Edit</a>
                            <?php
                            if($row['V_Status']=='Active')
                            {
                            ?>
                            <a href="vendorremove.php?user_id=<?php echo $row['Vendor_id'];?>" type="submit" style="font-size: small;" class="btn btn-outline-danger">Deactivate</a>
                            <?php
                            }
                            else
                            {
                            ?>
                            <a href="vendorremove.php?user_id=<?php echo $row['Vendor_id'];?>" type="submit" style="font-size: small;" class="btn btn-outline-success">Activate</a>
                            <?php
                            }
                            ?>
                          </td>
